                                  ...
                                  <div key={idx} className="bg-[#111111] border border-white/5 rounded-3xl p-6 relative overflow-hidden group">
                                     ...
                                     <div className="relative z-10">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                           <div className="space-y-2">
                                              <label className="text-[9px] font-black uppercase text-gray-500 ml-1">At Numaraları Seçimi</label>
                                              <div className="flex flex-wrap gap-2">
                                                {Array.from({ length: 30 }, (_, i) => i + 1).map(num => {
                                                  const isSelected = ayak.horses.split(/[,/\s-]+/).includes(num.toString());
                                                  return (
                                                    <button
                                                      key={num}
                                                      type="button"
                                                      onClick={() => {
                                                         const currentHorses = ayak.horses.split(/[,/\s-]+/).filter(Boolean);
                                                         let newSet;
                                                         if (currentHorses.includes(num.toString())) {
                                                           newSet = currentHorses.filter(h => h !== num.toString());
                                                         } else {
                                                           newSet = [...currentHorses, num.toString()].sort((a,b)=> Number(a) - Number(b));
                                                         }
                                                         const newAyaklar = [...formData.ayaklar];
                                                         newAyaklar[idx].horses = newSet.join(', ');
                                                         setFormData({...formData, ayaklar: newAyaklar});
                                                      }}
                                                ...
                                                <button
                                                  ...
                                                  onClick={() => {
                                                     const newAyaklar = [...formData.ayaklar];
                                                     if (newAyaklar[idx].horses.includes('Hepsi')) {
                                                        newAyaklar[idx].horses = newAyaklar[idx].horses.replace(/(?:,\s*)?Hepsi(?:,\s*)?/g, '');
                                                     } else {
                                                        newAyaklar[idx].horses = newAyaklar[idx].horses ? newAyaklar[idx].horses + ', Hepsi' : 'Hepsi';
                                                     }
                                                     setFormData({...formData, ayaklar: newAyaklar});
                                                  }}
                                                  className={`px-3 h-8 rounded-lg flex items-center justify-center text-xs font-black transition-all ${
                                                    ayak.horses.includes('Hepsi') 
                                                      ? 'bg-green-500 text-white shadow-[0_0_10px_rgba(34,197,94,0.5)]' 
                                                      : 'bg-[#ffcc00] text-black shadow-[0_0_10px_rgba(255,204,0,0.5)]'
                                                  }`}
                                                >
                                                  HEPSİ
                                                </button>
                                              ...
                                              <input 
                                                type="text"
                                                placeholder="Örn: 2, 4, 6, 8, 10 veya Hepsi"
                                                value={ayak.horses}
                                                onChange={(e) => {
                                                   const newAyaklar = [...formData.ayaklar];
                                                ...
                                                className="w-full bg-black/40 border border-white/5 rounded-xl p-4 text-xs font-bold text-green-400 focus:border-green-400 outline-none transition-colors mt-2"
                                              />
                                           </div>
                                           <div className="space-y-2">
